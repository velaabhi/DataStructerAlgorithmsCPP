...
class LTMatrix{
    ...
    public:
            LTMatrix(int n)     //constructor - allocates memory
            {
                  this->n = n;
                  A = new int [n*(n+1)/2];  
            }

            ~LTMatrix (){       //destructor - deallocates mem
                delete[] A;
            }

    void Display(bool row=true);    //if true then print in RMO, else in CMO
...
int main()
{
    ...
    LTMatrix cm(4); //for col major matrix
